...
func (s *service) ApplyEndpoint(ctx context.Context, domainEp *xdsmodels.DomainEndpoint) error {
	...
	g.Go(func() error {
		...
		listenerSafe := xdsconverter.ListenerConverter(domainEp, cert, s.protectedListenerPort)
	...
	// UNSAFE LISTENER
	g.Go(func() error {
		listenerUnsafe := xdsconverter.ListenerConverter(domainEp, nil, s.unprotectedListenerPort)
