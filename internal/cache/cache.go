...
func InitializeCache(ctx context.Context, certClient certclient.Client, routesClient routesclient.Client, protectedListenerPort, unprotectedListenerPort uint32, logger *slog.Logger) (cdsCache *cache.LinearCache, ldsCache *cache.LinearCache, rdsCache *cache.LinearCache, err error) {
	...
	for domainEp := range domainEpCh {
		...
		g.Go(func() error {
			...
			listenerSafe := xdsconverter.ListenerConverter(domainEp, cert, protectedListenerPort)
		...
		//UNSAFE LISTENER
		g.Go(func() error {
			listenerUnsafe := xdsconverter.ListenerConverter(domainEp, nil, unprotectedListenerPort)
